...
def split_dataSet(train_fold, dev_fold, trial_fold):
    ...
    return trainX, trainY, supportX, supportY
...
class Job:
    def __init__(self, seed=4568):
        self.log_file = utils.set_logger("./train_v9.log")
        ...
        self.warm_ratio = 0.1
        self.model_dir = "Zero_Shot_XLMRoberta_Large/End2endXLMRoBertaNet_large_v5"
        logging.info("实验五:最优的随机数:{},epoch:{},使用knn预测gold文件".format(self.epoches,self.seed))
        self.trainX, self.trainY, self.supportX, self.supportY = split_dataSet(self.train_fold,self.dev_fold,self.trial_fold)
    ...
    def few_shot_train(self, submis_fold=None):
        ...
        with torch.no_grad():
            ...
            logging.info(
                "knn few-shot 在训练集(dev/test)上 f1:{}, acc:{}".format(f1, acc))
            ...
            for json_fold in os.listdir(self.test_fold):
                for f in os.listdir(os.path.join(self.test_fold,json_fold)):
                    ...
                    if json_file.endswith(".data") == False:
                        continue
                    label_file = os.path.join(submis_fold,f[0:-5])
                    if os.path.exists(submis_fold) == False:
                        os.makedirs(submis_fold)
    ...
    def train(self):
        ...
        valid_data = MyData.from_list(self.trainX, self.trainY)

        logging.info("训练集数目:{}".format(len(train_data)))

        train_dataloader = DataLoader(dataset=train_data, sampler=RandomSampler(
            train_data), batch_size=self.batch_size, shuffle=False, drop_last=False,collate_fn=collate_func,num_workers=8)
        valid_dataloader = DataLoader(
...
if __name__ == "__main__":
    ...
    # job.train()
    # job.finetune()
    job.few_shot_train(submis_fold="few_shot_v5_test_submission")
    # job.finetune_evaluate()
    # job.submiss_fold("few_shot_v5_test_submission")
