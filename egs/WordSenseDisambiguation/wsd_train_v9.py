...
def split_dataSet2(train_fold, dev_fold):
    ...
    testX, testY = [], []
    for json_file in os.listdir(dev_fold):
        if json_file.endswith('.data') and ('zh-zh' in json_file):
            data_json = os.path.join(dev_fold, json_file)
            tags_json = os.path.join(dev_fold, json_file[0:-5]+".gold")
            inputX, target = load_dataSet(data_json, tags_json)
...
class Job:
    def __init__(self, seed=4568):
        ...
        self.trial_fold = r"dataset/trial/crosslingual"
        self.seed = seed
        ...
        self.warm_ratio = 0.1
        self.model_dir = "End2endXLMRoBertaNet_large_base_{}".format(self.seed)
        ...
        # self.trainX, self.trainY, self.validX, self.validY = split_dataSet2(self.train_fold,self.dev_fold)
        # utils.setup_seed(seed)

    def finetune(self, mode='froze_bert'):
        
        best_model_dir = os.path.join(self.model_dir, "best.pth.tar")
        support_set = split_dataSet(self.train_fold, self.trial_fold)
        for name, (trainX, trainY) in support_set.items():
            print("name:{}".format(name))
            finetune_output = "Few_shot_Roberta_large/End2end_roberta_large_finetune_{}".format(name)
            train_data = MyData.from_list(trainX, trainY)
            valid_data = MyData.from_list(trainX, trainY)
    ...
    def train(self):
        self.trainX, self.trainY, self.validX, self.validY = split_dataSet2(
        ...
        optimizer = torch.optim.SGD(
            [
                {"params": model.bert_model.parameters(), "lr": 2.5e-5},
                {"params": base_params},
            ],
...
if __name__ == "__main__":
    ...
    # job.train()
    # job.model_test()
    job.submiss_fold( "Few_Shot_XLMRoberta_large_v2_test_submission")
