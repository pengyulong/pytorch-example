...
class Job:
    def __init__(self, seed=4568):
        self.log_file = utils.set_logger("./few_shot_v2.log")
        ...
        self.trainX, self.trainY, self.support_set = split_dataSet(
            self.train_fold, self.trial_fold)
    ...
    def submiss_fold(self, outfold):
        model = WordDisambiguationNet(
            bert_model=bert_model, bert_tokenizer=bert_tokenizer, in_features=self.in_features)
        test_fold = os.path.join(self.test_fold,"crosslingual")
        newfold = os.path.join(outfold,"crosslingual")
        if os.path.exists(newfold) == False:
            os.makedirs(newfold)
        for files in os.listdir(test_fold):
            text_json = os.path.join(test_fold,files)
            name = extract_name(files)
            texts, ids = load_text_json(text_json)
            finetune_model = r"Few_Shot_v2/End2endXLMRobertaNet_v4_finetune_{}".format(name)
            result_list = []
...
if __name__ == "__main__":
    ...
    # job.support_set()
    job.submiss_fold("few_shot_v2_test_submission")
    # job.train()
    # job.model_test()
