...
def split_dataSet(train_fold, dev_fold, trial_fold):
    ...
    for json_file in os.listdir(dev_fold):
        if json_file.endswith('.data'):
            ...
            logging.info("json_file:{},names:{}".format(json_file,names))
            inputX, target = load_dataSet(data_json, tags_json)
            train_validX, testX2, train_validY, testY2 = train_test_split(
                inputX, target, test_size=0.2, random_state=0)
            trainX2, validX2, trainY2, validY2 = train_test_split(train_validX,train_validY,test_size=0.25,random_state=0)
            test_set[names] = (testX2,testY2)
            validX.extend(validX2)
            validY.extend(validY2)
            trainX.extend(trainX2)
            trainY.extend(trainY2)
    return trainX, trainY, validX, validY, supportX, supportY, test_set
...
class Job:
    def __init__(self, seed=4568):
        self.log_file = utils.set_logger("./train_v8.log")
        ...
        self.warm_ratio = 0.1
        self.model_dir = "Zero_Shot_XLMRoberta_Large/End2endXLMRoBertaNet_large_v4_{}".format(self.seed)
        logging.info("实验四:用来寻找最优的随机数种子")
        self.trainX, self.trainY, self.validX, self.validY, self.supportX, self.supportY, self.test_set = split_dataSet(self.train_fold,self.dev_fold,self.trial_fold)
        logging.info("当前随机数种子:{}".format(self.seed))
        

    def finetune(self, mode='froze_bert'):
        best_model_dir = os.path.join(self.model_dir, "best.pth.tar")
        finetune_output = "Few_shot_Roberta_large/End2end_roberta_large_finetune_{}".format(self.seed)
        train_data = MyData.from_list(self.supportX, self.supportY)
        valid_data = MyData.from_list(self.supportX, self.supportY)
    ...
    def finetune_evaluate(self):
        for names, (testX,testY) in self.test_set.items():
            ...
            logging.info("finetune 在测试集(dev/test){}上的性能指标f1:{},acc:{}".format(names,f1_score(y_trues,
                                                                                        y_preds), accuracy_score(y_trues, y_preds)))

    def few_shot_train(self, submis_fold=None):
        device = utils.get_device()
        train_data = MyData.from_list(self.supportX, self.supportY)
        logging.info("----------few-shot knn 训练集样本数:{}-------".format(len(train_data)))
        ...
        with torch.no_grad():
            ...
            for names,(testX,testY) in self.test_set.items():
                test_data = MyData.from_list(testX,testY)
                valid_dataloader = DataLoader(dataset=test_data, batch_size=10, shuffle=False, num_workers=0, collate_fn=collate_func, drop_last=False)
                ...
                logging.info(
                    "knn few-shot 在验证集{}(dev/test)上 f1:{}, acc:{}".format(names,f1, acc))
    ...
    def train(self):
        ...
        train_dataloader = DataLoader(dataset=train_data, sampler=RandomSampler(
            train_data), batch_size=self.batch_size, shuffle=False, drop_last=False,collate_fn=collate_func,num_workers=4)
        valid_dataloader = DataLoader(
            dataset=valid_data, batch_size=self.batch_size, shuffle=False, collate_fn=collate_func, drop_last=False)
        model = WordDisambiguationNet(
            bert_model=self.bert_model, bert_tokenizer=self.bert_tokenizer, in_features=self.in_features)
        model.to(device=self.device)
...
if __name__ == "__main__":
    for seed in [2020,1234,4568,6893,2235]:
