...
class Job:
    def __init__(self, seed=4568):
        self.log_file = utils.set_logger("./few_shot_v1.log")
        ...
        self.trainX, self.trainY, self.support_set, self.valid_set, self.test_set = split_dataSet(
    ...
    def finetune(self, mode='froze_bert'):
        ...
        for name,(trainX,trainY) in self.support_set.items():
            finetune_output = "End2endXLMRobertaNet_v3_finetune_{}".format(name)
            (validX, validY) = self.valid_set[name]
            logging.info("finetune {}, 训练集样本数:{},验证集样本数:{}".format(name,
            len(trainY), len(validY)))
            train_data = MyData.from_list(trainX, trainY)
            valid_data = MyData.from_list(validX, validY)
            train_dataloader = DataLoader(dataset=train_data, sampler=RandomSampler(train_data), batch_size=20, shuffle=False, num_workers=4, drop_last=False, collate_fn=collate_func, pin_memory=True)
            valid_dataloader = DataLoader(dataset=valid_data, batch_size=20, shuffle=False, num_workers=0, collate_fn=collate_func, drop_last=False)
    ...
    def predict(self,model_dir,text_json):
        result_list = []
        model = WordDisambiguationNet(
            bert_model=bert_model, bert_tokenizer=bert_tokenizer, in_features=self.in_features)
        utils.load_checkpoint(model_dir, "best.pth.tar"), model)
        model.to(device=self.device)
        model.eval()
        result_list = []
        with torch.no_grad():
    ...
    def submiss_fold(self, outfold):
        model = WordDisambiguationNet(
            bert_model=bert_model, bert_tokenizer=bert_tokenizer, in_features=self.in_features)
        for name,_ in self.valid_set.items():
            finetune_model = "End2endXLMRobertaNet_v3_finetune_{}".format(name)
...
if __name__ == "__main__":
    ...
    # job.support_set()
    job.submiss_fold("test_few_shot_v1")
    # job.train()
    # job.model_test()
