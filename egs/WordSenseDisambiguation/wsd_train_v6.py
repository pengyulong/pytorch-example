...
def split_dataSet(train_fold, trial_fold):
    trainX, trainY = load_dataSet(os.path.join(
        train_fold, "training.en-en.data"), os.path.join(train_fold, "training.en-en.gold"))
    support_set = {}
    for json_fold in os.listdir(trial_fold):
        for json_file in os.listdir(os.path.join(trial_fold,json_fold)):
            if json_file.endswith('.data'):
                data_json = os.path.join(trial_fold,json_fold, json_file)
                tags_json = os.path.join(trial_fold,json_fold, json_file[0:-5]+".gold")
                inputX, target = load_dataSet(data_json, tags_json)
                language_name = extract_name(json_file)
                support_set[language_name] = (inputX,target)
    return trainX, trainY, support_set


def split_dataSet2(train_fold, dev_fold):
    trainX, trainY = load_dataSet(os.path.join(
        train_fold, "training.en-en.data"), os.path.join(train_fold, "training.en-en.gold"))
    support_set = {}
    for json_file in os.listdir(dev_fold):
        if json_file.endswith('.data'):
            ...
            x_train,y_train = load_dataSet(data_json,tags_json)
            language_name = extract_name(json_file)
            support_set[language_name] = (x_train,y_train)
    return trainX, trainY, support_set


def split_dataSet3(train_fold, dev_fold, mode='v1'):
    trainX, trainY = load_dataSet(os.path.join(
        train_fold, "training.en-en.data"), os.path.join(train_fold, "training.en-en.gold"))
    testX, testY = [], []
    for json_file in os.listdir(dev_fold):
...
def train_and_evaluate(model, train_dataloader, val_dataloader, optimizer, loss_func, metrics, epochs, model_dir, lr_scheduler, restore_file=None):
    ...
    for epoch in range(epochs):
        ...
        train_metrics = train(model, optimizer, loss_func,
...
class Job:
    def __init__(self, seed=4568):
        self.log_file = utils.set_logger("./few_shot_v4.log")
        ...
        self.warm_ratio = 0.1
        self.model_dir = "End2endXLMRoBertaNet_v3_4568"
        self.trainX, self.trainY, self.support_set = split_dataSet2(
    ...
    def finetune(self, mode='froze_bert'):
        ...
        for name,(trainX,trainY) in self.support_set.items():
            finetune_output = "End2endXLMRobertaNet_v4_finetune_{}".format(name)
            # (validX, validY) = self.valid_set[name]
            logging.info("finetune {}, 训练集样本数:{}".format(name,len(trainY)))
            train_data = MyData.from_list(trainX, trainY)
            train_dataloader = DataLoader(dataset=train_data, sampler=RandomSampler(train_data), batch_size=len(train_data), shuffle=False, num_workers=4, drop_last=False, collate_fn=collate_func, pin_memory=True)
            model = WordDisambiguationNet(bert_model=self.bert_model, bert_tokenizer=self.bert_tokenizer, in_features=self.in_features)
            ...
            lr_scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=self.warm_ratio*(4*len(train_dataloader)), num_training_steps=self.epoches*len(train_dataloader))
            criterion = nn.CrossEntropyLoss()
            self.loss_result = train_and_evaluate(model, train_dataloader,None, optimizer, criterion,
            ...
            utils.save_dict_to_json(curr_hyp, os.path.join(
                finetune_output, "train_hyp.json"))

    def finetune_evaluate(self):
        for name,(testX,testY) in self.test_set.items():
            logging.info("{}测试集样本数:{}".format(name,len(testY)))
            ...
            utils.load_checkpoint(os.path.join(
                finetune_output, "best.pth.tar"), model)
            y_preds, y_trues = [], []
            model.eval()
            with torch.no_grad():
            ...
            logging.info("finetune 在测试集{}(dev/test)上的性能指标f1:{},acc:{}".format(name,f1_score(y_trues,
    ...
    def submiss_fold(self, outfold):
        model = WordDisambiguationNet(
            bert_model=bert_model, bert_tokenizer=bert_tokenizer, in_features=self.in_features)
        for name,_ in self.support_set.items():
            finetune_model = "End2endXLMRobertaNet_v4_finetune_{}".format(name)
            utils.load_checkpoint(os.path.join(finetune_model, "best.pth.tar"), model)
            model.to(device=self.device)
            model.eval()
            result_list = []
            with torch.no_grad():
                for files in os.listdir(os.path.join(self.test_fold,"multilingual")):
                    newfold = os.path.join(outfold,"multilingual")
                    if os.path.exists(newfold) == False:
                        os.makedirs(newfold)
                    text_json = os.path.join(self.test_fold, "multilingual", files)
                    if text_json.endswith('.data'):
...
if __name__ == "__main__":
    ...
    # job.support_set()
    job.submiss_fold("Few_Shot_v4_finetune")
    # job.train()
    # job.model_test()
