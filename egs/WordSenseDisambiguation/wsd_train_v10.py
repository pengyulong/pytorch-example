...
def split_dataSet(train_fold, dev_fold, trial_fold):
    ...
    return trainX, trainY, validX, validY, supportX, supportY, testX, testY
...
class Job:
    def __init__(self, seed=4568):
        self.log_file = utils.set_logger("./train_v5.log")
        ...
        self.warm_ratio = 0.1
        self.model_dir = "Zero_Shot_XLMRoberta_Large/End2endXLMRoBertaNet_large_v1"
        logging.info("实验一:用来比较knn和finetune")
        self.trainX, self.trainY, self.validX, self.validY, self.supportX, self.supportY, self.testX, self.testY = split_dataSet(self.train_fold,self.dev_fold,self.trial_fold)
        

    def finetune(self, mode='froze_bert'):
        
        best_model_dir = os.path.join(self.model_dir, "best.pth.tar")
        finetune_output = "Few_shot_Roberta_large/End2end_roberta_large_finetune_v5"
