...
from utils import load_checkpoint, classify_metrics, setup_seed, set_logger, get_device, train_and_evaluate, save_dict_to_json, split_dataSet, read_sequence_data, RunningAverage
...
class SequenceData(Dataset):
    def __init__(self, dataX1, dataX2, dataX3, dataX4, dataX5, dataX6, dataX7, dataY):
        ...
        self.dataY = F.one_hot(torch.from_numpy(dataY))
...
class Job:
    def __init__(self):
        ...
        self.trainX7, self.trainY, self.testX7, self.testY, seq_length7 = read_sequence_data(
    ...
    def train(self):
        ...
        valid_loss, train_loss = train_and_evaluate(
            model, train_dataloader, valid_dataloader, optim, criterion, metrics, self.epoches, model_dir=self.model_dir, restore_file=None)
    ...
    def predict(self):
        ...
        with torch.no_grad():
            for batch in valid_dataloader:
                ...
                y_true = np.argmax(
                    label_batch.detach().cpu().numpy(), axis=1).squeeze()
                result = pd.DataFrame(
